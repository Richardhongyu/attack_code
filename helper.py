...
def evaluate_models(models, x_test, y_test):
    ...
    for model in models:
        ...
        # print(predictions[1])
        # print(predictions)
        np_shape = np.array(predictions)
        print(np_shape.shape)
        # print(predictions)
        print('00000000000000000000000000000000000000000000000000')
        correct = [[model.name, i, label, np.max(pred), pred] for i, (label, pred) in
                   enumerate(zip(y_test[:, 0], predictions))
                   if label == np.argmax(pred)]
        accuracy = len(correct) / len(x_test)
        print('-----------------------------------------')
        print(accuracy, len(correct), len(x_test))
        print('-----------------------------------------')
